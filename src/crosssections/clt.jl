...
"""
    OrthotropicMaterial(E1, E2, nu12, G12, rho, name="")

A homogeneous orthotropic material in a plane stress state.
...
- `nu12::float`: Poisson's ratio  (nu12*E2 = nu21*E1)
- `rho::float`: density
- `name::string`: optional identifier
"""
struct OrthotropicMaterial{TF, TS}
    E1::TF
    E2::TF
    nu12::TF
    G12::TF
...
OrthotropicMaterial(E1, E2, nu12, G12, rho) = OrthotropicMaterial(E1, E2, nu12, G12, rho, "")
...
- `material::OrthotropicMaterial`: corresponding material
"""
struct Lamina{TF, TM}
    t::TF
    theta::TF
...
function Qbar(lamina)
    ...
    Q = Symmetric([E1*delta  nu12*E2*delta  0;
                  nu12*E2*delta  E2*delta  0;
                  0  0  G12])
    Ts = [c2 s2 -2*cs;
          s2 c2 2*cs;
          cs -cs c2-s2]
...
function clt(laminate, forces, strength)
    ...
    failure = tsai_wu(sigma, strength)
...
function tsai_wu(sigma, strength)
    S1t = strength.S1t
    S1c = strength.S1c
    S2t = strength.S2t
    S2c = strength.S2c
    ...
    failure = zeros(n)  # fails if > 1
    for i = 1:n
        failure[i] = sigma[1, i]^2/(S1t*S1c) + sigma[2, i]^2/(S2t*S2c) - sigma[1, i]*sigma[2, i]/sqrt(S1t*S1c*S2t*S2c) +
            sigma[1, i]*(1/S1t - 1/S1c) + sigma[2, i]*(1/S2t - 1/S2c) + sigma[3, i]^2/S12^2
    end
