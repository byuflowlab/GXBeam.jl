...
function xpfunc_steady_benchmark()
    ...
    # nonlinear steady state via the matrix-free GMRES path
    # (passing xpfunc routes through static_matrixfree_nlsolve! → matrixfree_nlsolve!,
    # which uses SparseDiffTools.JacVec as the GMRES matvec and the analytical
    # Jacobian as a left preconditioner).
    system, state, converged = steady_state_analysis(assembly,
        linear_velocity = [0, 196.349540849362, 0],
        angular_velocity = [0, 0, 78.5398163397448],
        prescribed_conditions = prescribed_conditions,
...
function xpfunc_unsteady_benchmark()
    ...
    # time marching via the matrix-free GMRES path
    system, history, converged = time_domain_analysis(assembly, tvec;
        linear_velocity = [0, 196.349540849362, 0],
        angular_velocity = [0, 0, 78.5398163397448],
        prescribed_conditions = prescribed_conditions,
...
println("Steady State Analysis (xpfunc / matrix-free GMRES path): ")
@btime xpfunc_steady_benchmark();

# TODO: re-enable once upstream is fixed.
# `time_domain_analysis(...; xpfunc=...)` currently crashes with
# UndefVarError(:x) at src/analyses.jl:3360 — `newmark_parameters`
# unpacks xpfunc but not x from constants, then calls xpfunc(x, p_p, t).
# println("Time Marching Analysis (xpfunc / matrix-free GMRES path): ")
# @btime xpfunc_unsteady_benchmark();
